							...
							TransactionId *global_visibility_cutoff_xid)
{
	...
	for (; tupindex < vacrelstats->dead_tuples->num_tuples; tupindex++)
	{
		...
		toff = ItemPointerGetOffsetNumber(&vacrelstats->dead_tuples->itemptrs[tupindex]);
		unused[uncnt++] = toff;
	}
					  ...
					  TransactionId visibility_cutoff_xid)
{
	...
	for (; idx < vacrelstats->dead_tuples->num_tuples; idx++)
	{
		BlockNumber tblk;
		...
		tblk = ItemPointerGetBlockNumber(&vacrelstats->dead_tuples->itemptrs[idx]);
				...
				BufferAccessStrategy vac_strategy, bool aggressive)
{
	...
	initprog_val[2] = vacrelstats->dead_tuples->max_tuples;
	...
	for (blkno = ZHEAP_METAPAGE + 1; blkno < nblocks; blkno++)
	{
		 ...
		 */
		if ((vacrelstats->dead_tuples->max_tuples - vacrelstats->dead_tuples->num_tuples) < MaxZHeapTuplesPerPage &&
	...
	if (IsAutoVacuumWorkerProcess() && params->log_min_duration >= 0)
	{
									   ...
									   params->log_min_duration))
		{
			...
			appendStringInfo(&buf,
							 ...
							 OldestXmin);
			appendStringInfo(&buf,
							 _("buffer usage: %ld hits, %ld misses, %ld dirtied\n"),
							 VacuumPageHit,
							 VacuumPageMiss,
							 VacuumPageDirty);
