...
SELECT currtid('tid_matview'::regclass::oid, '(1,1)'::tid); -- fails
SELECT currtid2('tid_matview'::text, '(1,1)'::tid); -- fails
INSERT INTO tid_tab VALUES (1);
REFRESH MATERIALIZED VIEW tid_matview;
SELECT currtid('tid_matview'::regclass::oid, '(1,1)'::tid); -- ok
SELECT currtid2('tid_matview'::text, '(1,1)'::tid); -- ok
...
SELECT currtid('tid_ind'::regclass::oid, '(1,1)'::tid); -- fails
SELECT currtid2('tid_ind'::text, '(1,1)'::tid); -- fails
DROP INDEX tid_ind;

-- Partitioned table, no storage
CREATE TABLE tid_part (a int) PARTITION BY RANGE (a);
SELECT currtid('tid_part'::regclass::oid, '(1,1)'::tid); -- fails
SELECT currtid2('tid_part'::text, '(1,1)'::tid); -- fails
DROP TABLE tid_part;

-- Views
-- ctid not defined in the view
CREATE VIEW tid_view_no_ctid AS SELECT a FROM tid_tab;
SELECT currtid('tid_view_no_ctid'::regclass::oid, '(1,1)'::tid); -- fails
SELECT currtid2('tid_view_no_ctid'::text, '(1,1)'::tid); -- fails
...
SELECT currtid('tid_view_with_ctid'::regclass::oid, '(1,1)'::tid); -- fails
SELECT currtid2('tid_view_with_ctid'::text, '(1,1)'::tid); -- fails
INSERT INTO tid_tab VALUES (1);
SELECT currtid('tid_view_with_ctid'::regclass::oid, '(1,1)'::tid); -- ok
SELECT currtid2('tid_view_with_ctid'::text, '(1,1)'::tid); -- ok
...
SELECT currtid('tid_view_fake_ctid'::regclass::oid, '(1,1)'::tid); -- fails
SELECT currtid2('tid_view_fake_ctid'::text, '(1,1)'::tid); -- fails
