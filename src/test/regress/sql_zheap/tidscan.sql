...
EXPLAIN (COSTS OFF)
SELECT ctid, * FROM tidscan WHERE ctid = '(1,1)';
SELECT ctid, * FROM tidscan WHERE ctid = '(1,1)';

EXPLAIN (COSTS OFF)
SELECT ctid, * FROM tidscan WHERE '(1,1)' = ctid;
SELECT ctid, * FROM tidscan WHERE '(1,1)' = ctid;

-- OR'd clauses
EXPLAIN (COSTS OFF)
SELECT ctid, * FROM tidscan WHERE ctid = '(1,2)' OR '(1,1)' = ctid;
SELECT ctid, * FROM tidscan WHERE ctid = '(1,2)' OR '(1,1)' = ctid;

-- ctid = ScalarArrayOp - implemented as tidscan
EXPLAIN (COSTS OFF)
SELECT ctid, * FROM tidscan WHERE ctid = ANY(ARRAY['(1,1)', '(1,2)']::tid[]);
SELECT ctid, * FROM tidscan WHERE ctid = ANY(ARRAY['(1,1)', '(1,2)']::tid[]);
...
EXPLAIN (COSTS OFF)
SELECT ctid, * FROM tidscan
WHERE (id = 3 AND ctid IN ('(1,2)', '(1,3)')) OR (ctid = '(1,1)' AND id = 1);
SELECT ctid, * FROM tidscan
WHERE (id = 3 AND ctid IN ('(1,2)', '(1,3)')) OR (ctid = '(1,1)' AND id = 1);
...
SELECT * FROM tidscan WHERE ctid = '(1,1)';
